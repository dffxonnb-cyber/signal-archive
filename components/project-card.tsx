...
export function ProjectCard({
  ...
  const tools = (
    project.cardTools.length > 0 ? project.cardTools : project.stack
  ).slice(0, 4);
  const statusChips = [
  ...
  if (variant === "supporting") {
    return (
      ...
      >
        ...
        <div
          className="project-card__cta-row project-card__cta-row--compact"
          aria-label={`${project.title} links`}
        >
          {cardLinks.slice(0, 2).map((link) =>
            !link.external ? (
              <Link
                className="button-link project-card__cta project-card__cta--primary"
                href={link.href}
                key={link.href}
              >
                {link.label}
  ...
  return (
    ...
    >
      ...
      <div className="project-card__hero">
        <div
          className={`project-card__media project-card__media--${visual.tone}`}
        >
          {visual.mark}
        </div>
      ...
      <div className="project-card__evidence-panel">
        ...
        {metricChips.length > 0 ? (
          <div
            className="project-card__metric-strip"
            aria-label={`${project.title} key metrics`}
          >
            {metricChips.slice(0, 3).map((item) => (
              <span
                className="project-card__metric-pill"
        ...
        {operationChips.length > 0 ? (
          <div
            className="project-card__operation-strip"
            aria-label={`${project.title} operation metadata`}
          >
            {operationChips.map((item) => (
              <span
                className="project-card__operation-pill"
                key={`${item.label}-${item.value}`}
              >
                <span>{item.label}</span>
                <strong>{item.value}</strong>
      ...
      <div
        className="project-card__proof-grid"
        aria-label={`${project.title} hiring review fields`}
      >
      ...
      <div className="project-card__footer">
        ...
        <div
          className="project-card__cta-row"
          aria-label={`${project.title} links`}
        >
          {cardLinks.map((link) =>
            !link.external ? (
